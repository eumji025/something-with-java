...
/**
 * copy from cxf the {@link ThreadPoolExecutor} more implement
 *
 * to solve when {@link ThreadPoolExecutor} core thread has used
 * it must wait waiter queue was full, more worker（thred） can be create to help handler wait queue job
 */
public class AutomaticWorkQueueImpl implements AutomaticWorkQueue {
    ...
    public AutomaticWorkQueueImpl(int mqs, 
        ...
        addWorkerStep = maxQueueSize / (highWaterMark-lowWaterMark);
...
//                    }
                    ...
                    int addWorker = sz % addWorkerStep == 0 ? sz / addWorkerStep : sz /addWorkerStep + 1;

                    //可用的小于最大值，并且需要的工作线程比现有的线程多
                    if (sz2 < highWaterMark && lowWaterMark+addWorker > ps) {
                        ReflectionUtil.setAccessible(addWorkerMethod).invoke(executor, addWorkerArgs);
                        //System.out.println(String.format("当前队列存在：%d 个任务，步长：%d 期望工作线程：%d个，实际已有线程：%d个",sz,addWorkerStep,lowWaterMark+addWorker,ps));
                    }
                } catch (Exception exc) {
                    //ignore
                } finally {
